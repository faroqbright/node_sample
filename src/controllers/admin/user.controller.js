...
import { getJWTToken, randomValueHex, getEncryptedPassword, decode, decodeToken, makeid, sendNotification } from '../../libraries/util';
...
export default {
    ...
    async editProfile(req, res) {
        try {
            ...
            if (!name) {
                let result = makeApiResponce('Name is required', 1, BAD_REQUEST)
                return res.status(BAD_REQUEST).json(result);
            }
            if (!email) {
                let result = makeApiResponce('Email is required', 1, BAD_REQUEST)
                return res.status(BAD_REQUEST).json(result);
            }
            if (!phoneNumber) {
                let result = makeApiResponce('Phone Number is required', 1, BAD_REQUEST)
                return res.status(BAD_REQUEST).json(result);
            }
            const pre = await staffModel.findOne({ email })
            if (pre && pre.email !== findUser.email) {
                let result = makeApiResponce('Email is taken! Please choose another email.', 1, BAD_REQUEST)
                return res.status(BAD_REQUEST).json(result);
            }
            ...
            if (password) {
                if (!(/^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*])[0-9a-zA-Z!@#$%^&*]*$/).test(password)) {
                    let result = makeApiResponce('Password must contain One Uppercase, One Lowercase, One Digit, one special, Min 8 characters in Length', 1, BAD_REQUEST)
                    return res.status(BAD_REQUEST).json(result);
                }
