...
export default {
    ...
    async bagRequest(req, res) {
        try {
            ...
            const { error, value } = mobileService.validateRequestNewBagRequest(req.body);
            if (error && error.details) {
                let result = makeApiResponce(error.details[0].message, 0, BAD_REQUEST)
                return res.status(BAD_REQUEST).json(result);
            }
            ...
            let notificationModel = new NotificationModel();
            notificationModel.user = findUser._id;
            notificationModel.title = "New bag request";
            notificationModel.body = "A new bag request has been sent to your company";
            notificationModel.type = "newBagRequest";
            await notificationModel.save();
            let notiRecievreList = []
